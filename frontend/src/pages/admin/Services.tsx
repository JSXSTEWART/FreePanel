...
export default function Services() {
  ...
  const handleStart = async (serviceId: string) => {
    try {
      ...
      toast.error(axiosError.response?.data?.message || 'Failed to start service')
    } finally {
  ...
  const handleStop = async (serviceId: string) => {
    try {
      ...
      const axiosError = error as { response?: { data?: { message?: string } } }
      toast.error(axiosError.response?.data?.message || 'Failed to stop service')
    } finally {
      setActionLoading(null)
    }
  }
  ...
  const handleRestart = async (serviceId: string) => {
    try {
      ...
      const axiosError = error as { response?: { data?: { message?: string } } }
      toast.error(axiosError.response?.data?.message || 'Failed to restart service')
    } finally {
      setActionLoading(null)
    }
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardBody className="flex items-center">
            ...
            <div>
              <div className="text-2xl font-bold text-gray-900">{runningCount}</div>
        ...
        <Card>
          <CardBody className="flex items-center">
            ...
            <div>
              <div className="text-2xl font-bold text-gray-900">{stoppedCount}</div>
                ...
                services.map((service) => (
                  <tr key={service.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="font-medium text-gray-900">{service.display_name}</div>
                      ...
                      <div className="flex space-x-2">
                        {service.is_running ? (
                          <>
                            ...
                            >
                              <ArrowPathIcon className={`w-4 h-4 mr-1 ${actionLoading === `${service.id}-restart` ? 'animate-spin' : ''}`} />
                              Restart
                            </button>
                            <button
        ...
        <CardBody>
          ...
          <div className="flex flex-wrap gap-3">
            <Button
              variant="secondary"
              onClick={async () => {
                const webServices = services.filter(s => ['httpd', 'nginx', 'php-fpm'].includes(s.id))
            ...
            <Button
              variant="secondary"
              onClick={async () => {
                const mailServices = services.filter(s => ['dovecot', 'exim', 'postfix'].includes(s.id))
            ...
            <Button
              variant="secondary"
              onClick={async () => {
                const dbServices = services.filter(s => ['mariadb', 'mysql', 'postgresql', 'redis'].includes(s.id))
