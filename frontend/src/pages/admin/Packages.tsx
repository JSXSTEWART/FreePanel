    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">Define hosting plans with resource limits</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {packages.length === 0 ? (
          ...
          packages.map((pkg) => (
            <Card key={pkg.id} className={pkg.is_default ? 'ring-2 ring-primary-500' : ''}>
              <CardBody>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">{pkg.name}</h3>
                    {pkg.is_default && (
                      <span className="text-xs text-primary-600 font-medium">Default Package</span>
                    )}
                  </div>
                  <div className="flex space-x-1">
                    ...
                    <button
                      ...
                      disabled={actionLoading === pkg.id || pkg.is_default || (pkg.accounts_count || 0) > 0}
                    >
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-gray-900">{formatLimit(pkg.disk_quota, 'bytes')}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Bandwidth</span>
                    <span className="font-medium text-gray-900">{formatLimit(pkg.bandwidth, 'bytes')}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Domains</span>
                    <span className="font-medium text-gray-900">{formatLimit(pkg.max_addon_domains)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Subdomains</span>
                    <span className="font-medium text-gray-900">{formatLimit(pkg.max_subdomains)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Email Accounts</span>
                    <span className="font-medium text-gray-900">{formatLimit(pkg.max_email_accounts)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Databases</span>
                    <span className="font-medium text-gray-900">{formatLimit(pkg.max_databases)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">FTP Accounts</span>
                    <span className="font-medium text-gray-900">{formatLimit(pkg.max_ftp_accounts)}</span>
                  </div>
                </div>

                <div className="mt-4 pt-4 border-t border-gray-200">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">Accounts using this package</span>
                    <span className="text-lg font-semibold text-primary-600">{pkg.accounts_count || 0}</span>
                  </div>
                </div>
              </CardBody>
            </Card>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    {packages.map(pkg => (
                      <th key={pkg.id} className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">
                        {pkg.name}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  <tr>
                    <td className="px-6 py-3 text-sm font-medium text-gray-900">Disk Space</td>
                    {packages.map(pkg => (
                      <td key={pkg.id} className="px-6 py-3 text-sm text-center text-gray-500">
                        {formatLimit(pkg.disk_quota, 'bytes')}
                      </td>
                    ))}
                  </tr>
                  <tr>
                    <td className="px-6 py-3 text-sm font-medium text-gray-900">Bandwidth</td>
                    {packages.map(pkg => (
                      <td key={pkg.id} className="px-6 py-3 text-sm text-center text-gray-500">
                  ...
                  <tr>
                    <td className="px-6 py-3 text-sm font-medium text-gray-900">Addon Domains</td>
                    {packages.map(pkg => (
                      <td key={pkg.id} className="px-6 py-3 text-sm text-center text-gray-500">
                  ...
                  <tr>
                    <td className="px-6 py-3 text-sm font-medium text-gray-900">Email Accounts</td>
                    {packages.map(pkg => (
                      <td key={pkg.id} className="px-6 py-3 text-sm text-center text-gray-500">
                  ...
                  <tr>
                    <td className="px-6 py-3 text-sm font-medium text-gray-900">MySQL Databases</td>
                    {packages.map(pkg => (
                      <td key={pkg.id} className="px-6 py-3 text-sm text-center text-gray-500">
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, disk_quota: gbToBytes(parseInt(e.target.value) || 0) })}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, bandwidth: gbToBytes(parseInt(e.target.value) || 0) })}
                  />
                  <p className="text-xs text-gray-500 mt-1">-1 for unlimited</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, max_addon_domains: parseInt(e.target.value) || 0 })}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, max_subdomains: parseInt(e.target.value) || 0 })}
                  />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, max_email_accounts: parseInt(e.target.value) || 0 })}
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, max_ftp_accounts: parseInt(e.target.value) || 0 })}
                  />
                </div>
              </div>
              <div className="flex items-center">
                ...
                />
                <label htmlFor="isDefault" className="ml-2 text-sm text-gray-700">Set as default package</label>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
              ...
              >
                {actionLoading === -1 ? 'Saving...' : (editingPackage ? 'Update Package' : 'Create Package')}
              </Button>
            </div>
          </div>
        </div>
