...
export const getMe = async (): Promise<User> => {
  const response = await client.get<{ success: boolean; user: User }>(
    "/auth/me",
  );
...
// Get 2FA QR code
export const get2FAQRCode = async (): Promise<{
  secret: string;
  qr_code: string;
}> => {
...
export const getOAuthRedirectUrl = async (
  ...
  const response = await client.get<{ success: boolean; url: string }>(
