...
export const databasesApi = {
  ...
  create: async (data: { name: string }): Promise<Database> => {
    const response = await client.post<ApiResponse<Database>>('/databases', data)
    return response.data.data
  },

  get: async (id: number): Promise<Database> => {
    const response = await client.get<ApiResponse<Database>>(`/databases/${id}`)
    return response.data.data
  },
  ...
  grant: async (id: number, userId: number, privileges: string[]): Promise<void> => {
    await client.post(`/databases/${id}/grant`, { user_id: userId, privileges })
  },
  ...
  listUsers: async (): Promise<DatabaseUser[]> => {
    const response = await client.get<ApiResponse<DatabaseUser[]>>('/databases/users')
    return response.data.data
  },
  ...
  createUser: async (data: {
    ...
    const response = await client.post<ApiResponse<DatabaseUser>>('/databases/users', data)
    return response.data.data
  },

  getUser: async (id: number): Promise<DatabaseUser> => {
    const response = await client.get<ApiResponse<DatabaseUser>>(`/databases/users/${id}`)
    return response.data.data
  },
