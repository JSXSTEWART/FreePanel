...
export const sslApi = {
  list: async (): Promise<SslCertificate[]> => {
    const response = await client.get<ApiResponse<SslCertificate[]>>('/ssl/certificates')
    return response.data.data
  },

  get: async (id: number): Promise<SslCertificate> => {
    const response = await client.get<ApiResponse<SslCertificate>>(`/ssl/certificates/${id}`)
    return response.data.data
  },

  install: async (domain: string, data: {
    certificate: string
    private_key: string
    ca_bundle?: string
  }): Promise<SslCertificate> => {
    const response = await client.post<ApiResponse<SslCertificate>>('/ssl/certificates', {
      domain,
      ...data,
    })
    return response.data.data
  ...
  generateCsr: async (data: CsrRequest): Promise<{
    csr: string
    private_key: string
  }> => {
  ...
  requestLetsEncrypt: async (domain: string): Promise<SslCertificate> => {
    const response = await client.post<ApiResponse<SslCertificate>>('/ssl/lets-encrypt', { domain })
    return response.data.data
  },
