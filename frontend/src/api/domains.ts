...
export const domainsApi = {
  ...
  update: async (id: number, data: {
    ...
    const response = await client.put<ApiResponse<Domain>>(`/domains/${id}`, data)
    return response.data.data
  },
  ...
  getSubdomains: async (domainId: number): Promise<Subdomain[]> => {
    const response = await client.get<ApiResponse<Subdomain[]>>(`/domains/${domainId}/subdomains`)
    return response.data.data
  },

  createSubdomain: async (domainId: number, data: {
    name: string
    document_root?: string
  }): Promise<Subdomain> => {
    const response = await client.post<ApiResponse<Subdomain>>(`/domains/${domainId}/subdomains`, data)
    return response.data.data
  },

  deleteSubdomain: async (domainId: number, subdomainId: number): Promise<void> => {
    await client.delete(`/domains/${domainId}/subdomains/${subdomainId}`)
  },

  // Redirects
  addRedirect: async (domainId: number, data: {
    source: string
    destination: string
  ...
  removeRedirect: async (domainId: number, redirectId: number): Promise<void> => {
